...
if __name__ == "__main__":
    ...
    det_url = get_model_config(
        type="OCR", version=args.version, model_type="det", lang="en"
    )["url"]
    rec_url = get_model_config(
        type="OCR", version=args.version, model_type="rec", lang="latin"
    )["url"]
    cls_url = get_model_config(
        type="OCR", version=args.version, model_type="cls", lang="ch"
    )["url"]
    maybe_download(os.path.join(args.folder, paddle_settings.DETECTION_FOLDER), det_url)
    maybe_download(
        os.path.join(args.folder, paddle_settings.RECOGNITION_FOLDER), rec_url
    )
    maybe_download(
        os.path.join(args.folder, paddle_settings.CLASSIFICATION_FOLDER), cls_url
    )
