...
from src.schemas.output import DoclingDocument, MarkdownPage, OCRResult, Page
...
class OCRWorker(BaseWorker):
    ...
    def transform_content(self, task: TaskModel, content: bytes) -> list[Image.Image] | list[BytesIO]:
        ...
        if content_type.startswith("image/"):
            ...
            if self.ocr_model.__class__.__name__ != "DoclingInferOCR" or True: # TODO pour le moment, docling ne prend pas les pdf, que les images
    ...
    def predict_on_pages(self, task: TaskModel, pages: list[Image.Image | BytesIO]) -> TaskModel:
        ...
        for i in range(0, len(pages), batch_size):
            ...
            partial_result: list[DoclingDocument] | list[Page] = self.ocr_model.batch_predict(
            ...
            for j, page_or_doclingdocument in enumerate(partial_result):
                if isinstance(page_or_doclingdocument, Page):
                    assert isinstance(batch[j], Image.Image), f"{type(batch[j])=}"
                    pil_image = batch[j]
                    buffer = BytesIO()
                    ...
                    assert isinstance(page_or_doclingdocument, DoclingDocument)
                    doclingdocument: DoclingDocument = page_or_doclingdocument
                    for docling_page in doclingdocument.pages:
                        buffer = BytesIO()
                        docling_page.pil_image.save(buffer, format="PNG")
                        buffer.seek(0)
                        key = f"{task.user_id}/{task.id}/images/page_{i + j}.png"
                        client_s3.upload_fileobj(buffer, self.file_connector.bucket_name, key)
                        logger.debug(f"Uploaded page {i + j} to {key}")
                        signed_url = client_s3.generate_presigned_url(
                            ClientMethod="get_object",
                        ...
                        docling_page.page_url = signed_url
                        docling_page.pil_image = None
