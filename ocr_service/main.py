...
if __name__ == "__main__":
    ...
    minio_client = minio.Minio(endpoint=minio_settings.MINIO_END_POINT, access_key=minio_settings.MINIO_ACCESS_KEY, secret_key=minio_settings.MINIO_SECRET_KEY,
                               secure=False)
    ...
    while True:
        ...
        try:
            ...
            task = task_table.update_task(
                task_id=task.id, form_data=TaskUpdateForm(status="error", percentage=0, extras=extras))
            logger.error(str(e))
            continue
        ...
        if content is not None:
            ...
            task = task_table.update_task(
                task_id=task.id, form_data=TaskUpdateForm(status="on-going", percentage=0.1, extras=extras))
            extras = task.extras
            ...
            for i in range(0, len(pages), n):
                ...
                task = task_table.update_task(task_id=task.id, form_data=TaskUpdateForm(status="on-going", percentage=percentage, extras=extras
                                                                                        ))
            ...
            redis_client.set(name=f"{task.id}",
                             value=f"{task.model_dump()}", ex=3000)
