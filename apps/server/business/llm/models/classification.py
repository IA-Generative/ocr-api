...
class FormClassification(BaseLLMOCR):
    def __init__(
        self,
        ...
        self.prompt = "Est-ce un formulaire ?"
        self.batch_size = 2

    def process(self, image: Image.Image, batch_size: int = 2) -> ImageFormDetector:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                max_tokens=1000,
                response_model=ImageFormDetector,
            )
        except Exception as e:
            response = ImageFormDetector(is_form=False, confidence=0.0)
            logger.error(f"[model {self.__class__.__name__}] {str(e)}")

        return response

    async def aprocess(self, image_or_path: Union[Image.Image, Path], batch_size: int = 2) -> ImageFormDetector:
        ...
        async with semaphore:
            ...
            logger.info(f"[model {self.__class__.__name__}] processed in {time() - t:.2f}s")
            return response
    ...
    async def _async_batch_predict(self, images: list[Image.Image], pages: list[Page]):
        ...
        for page, result in zip(pages, results):
            page.image_form_detector = result
    ...
    def batch_predict(self, images: list[Image.Image], pages: list[Page] = [], *args, **kwargs):
        ...
        if isinstance(self.openai_client, OpenAI):
            for page, image in zip(pages, images):
                page.image_form_detector = self.process(image=image, batch_size=2)
