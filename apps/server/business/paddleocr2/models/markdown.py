...
class PPStructureInferV5(BaseModelPrediction):
    def __init__(self, **options):
        ...
        self.model: PPStructureV3 = PPStructureV3(
            ...
            use_doc_orientation_classify=options.get("use_doc_orientation_classify", True),
            use_table_recognition=options.get("use_table_recognition", True),
            use_doc_unwarping=options.get("use_doc_unwarping", False),
            use_formula_recognition=options.get("use_formula_recognition", False),
    ...
    def batch_predict(self, images: List[Image.Image], pages: list = [], *args, **kwargs) -> List[Page]:
        ...
        for i, image in enumerate(images):
            ...
            predictions = self.model.predict(
                np.array(image),
                use_chart_recognition=self._options.get("use_chart_recognition", False),
                use_formula_recognition=self._options.get("use_formula_recognition", False),
                use_ocr_results_with_table_cells=self._options.get("use_ocr_results_with_table_cells", True),
                use_doc_orientation_classify=self._options.get("use_doc_orientation_classify", True),
                use_doc_unwarping=self._options.get("use_doc_unwarping", False),
                use_textline_orientation=self._options.get("use_textline_orientation", False),
                use_region_detection=self._options.get("use_region_detection", True),
                use_seal_recognition=self._options.get("use_seal_recognition", False),
            )
            ...
            page = Page(page=i, boxes=page_boxes, markdown=pred.markdown.get("markdown_texts", None))
