...
from .lazy_zip import LazyZipList
...
class LazyEmailList(LazyFileImageList):
    ...
    La séquence est composite :
      ...
      - puis viennent les pages de chaque pièce jointe, rendues via
        ``LazyPdfImageList`` (PDF), ``LazyZipList`` (archives ZIP,
        dépliées récursivement) ou ``LazyFileImageList`` (autres formats).
        ...
        for att in self.attachments:
            ...
            try:
                if content_type == PDF_CONTENT_TYPE:
                    ...
                    # Archive ZIP : dépliée, une page (texte ou OCR) par
                    # fichier contenu, dans l'ordre alphabétique.
                    att["lazy"] = LazyZipList(att["path"], dpi=self.dpi, fmt=self.fmt)
                    att["kind"] = "zip"
                else:
            ...
            except Exception as exc:  # pièce jointe non rendable (archive corrompue, etc.)
                logger.warning(f"Impossible de rendre la pièce jointe {att['filename']} ({att['content_type']}): {exc}")
                att["lazy"] = None
                att["kind"] = "skip"
                att["page_count"] = 0
    ...
    def _append_attachment_sources(self, att: dict, global_idx: int) -> int:
        if att["kind"] == "text":
            return self._append_text_sources(att, global_idx)
        if att["kind"] == "zip":
            return self._append_zip_sources(att, global_idx)
        return self._append_ocr_sources(att, global_idx)

    def _append_text_sources(self, att: dict, global_idx: int) -> int:
    ...
    def _append_zip_sources(self, att: dict, global_idx: int) -> int:
        # Chaque page de l'archive a déjà sa propre source (texte ou OCR),
        # calculée par LazyZipList : on la reporte telle quelle.
        zip_lazy = att["lazy"]
        for offset in range(att["page_count"]):
            source = zip_lazy.page_sources[offset] if offset < len(zip_lazy.page_sources) else None
            self.page_sources.append(source)
            if offset in zip_lazy.ocr_pages:
                self.ocr_pages.add(global_idx)
            global_idx += 1
        return global_idx

    def _append_ocr_sources(self, att: dict, global_idx: int) -> int:
