...
def test_ocr_inference():
    obj = PPStructureInferV5(use_chart_recognition=True, use_formula_recognition=True,
                             use_region_detection=True, use_seal_recognition=False, use_doc_unwarping=True,
    ...
    for image_path in [image_path1, image_path2, image_path3]:
        ...
        actual_pages = obj.batch_predict(
            images=[image], save_markdown=f"tests/data/predictions/structure/{base_name}", save_visualization=f"tests/data/predictions/structure/{base_name}")
