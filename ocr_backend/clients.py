...
try:
    ...
    if redis_client.ping():
        health = Health(
            name="redis", version=redis.__version__, up_time=up_time, status="healthy"
        )
    else:
        health = HealthError(name="redis", error="Not Reachable", code_status=500)
