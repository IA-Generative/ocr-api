...
- componentValues: The component's values map (e.g. .Values.api).
*/}}
...
Parameters:
- root: The root context.
- componentEnv: The component's own `env` value (map or array), e.g. .Values.api.env.
*/}}
...
Parameters:
- root: The root context.
- componentEnvFrom: The component's own `envFrom` value (map or array), e.g. .Values.api.envFrom.
*/}}
...
{{/*
Render `frontend.viteEnv` as `window.KEY = value;`-style assignments, used by
`templates/frontend/vite-config.yaml` to build the frontend's runtime config.js (see
`frontend.viteEnv` in values.yaml).
*/}}
{{- define "helper.viteEnv" -}}
{{- range $key, $val := . }}
{{- if kindIs "bool" $val }}
{{ printf "%s.%s = %s" "window" $key ($val | toYaml) }}
{{- else }}
{{ printf "%s.%s = %s" "window" $key ($val | quote | default "''") }}
{{- end }}
{{- end }}
{{- end }}
...
(api/frontend/worker). Deployment and StatefulSet only differ in a handful of top-level
...
Parameters:
- root: The root context.
- name: The component name (e.g. "api"), used to derive its labels/CM/Secret/
  ServiceAccount name and to locate its configmap.yaml/secret.yaml for checksums.
- component: The component's values map (e.g. .Values.api).
*/}}
